...
namespace Graham_scan_WF
{
    public class Polygon
    {
        ...
        {
            if (pictureBox == null)
                throw new ArgumentNullException("Polygon.Polygon", "pictureBox");
        ...
        public void AddPoint(int x,int y)
        {
            if (x > pictureBox.Width)
                throw new ArgumentOutOfRangeException("Polygon.AddPoint", "x");
            if (y > pictureBox.Height)
                throw new ArgumentOutOfRangeException("Polygon.AddPoint", "y");
        ...
        {
            if (points == null)
                throw new ArgumentNullException("Polygon.Solve", "points");
            int min = points[0].X;
            ...
            //{
            //    if (i == points.Length && (points[0].X - points[i - 2].X) * (points[i - 1].Y - points[i - 2].Y) - (points[0].Y - points[i - 2].Y) * (points[i - 1].X - points[i - 2].X) > 0)
            //    {
            //        nicePoints.Add(points[i - 1]);
            //        break;
            //    }
            //    if ((points[i].X - points[i - 2].X) * (points[i - 1].Y - points[i - 2].Y) - (points[i].Y - points[i - 2].Y) * (points[i - 1].X - points[i - 2].X) > 0)
            //    {
            //        nicePoints.Add(points[i - 1]);
            //    }
            //}
            //nicepointsList = nicePoints;
